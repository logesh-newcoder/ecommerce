fetch('https://ecommerce-2-x700.onrender.com/api/mens/pant')
  .then(res => {
    if (!res.ok) throw new Error('Failed to fetch');
    return res.json();
  })
  .then(data => {
    const container = document.querySelector('.mens_pant_show');
    container.innerHTML = '';
  ...
  .catch(error => {
    ...
    const container = document.querySelector('.mens_pant_show');
    if (container) container.innerHTML = '<p style="color:red;">Failed to load Pants.</p>';
  });
